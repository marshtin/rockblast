...
# Obtener datos de la tabla fleet según el name y las dos consultas
def get_fleet_data_by_type(conn, name):
    if conn is None:
        raise Exception("Failed to connect to the database. Please check your configuration.")
    
    query_with_speed = f"SELECT * FROM sandbox.fleet WHERE name = '{name}' AND elevation != 0 AND speed > 0"
    query_without_speed = f"SELECT * FROM sandbox.fleet WHERE name = '{name}' AND elevation != 0 AND speed = 0"
...
def visualize_fleet_data_combined(df_with_speed_list, df_without_speed_list, name):
    traces = []
    ...
    if traces:
        layout = go.Layout(
            title=f'Datos del Fleet - Name: {name}',
            scene=dict(
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Available names:", type_names)
        
        processed_type_names = []  # Lista para almacenar los nombres de tipo procesados
        
        for name in type_names:
            print(f"Processing data for name: {name}")
            
            # Obtener datos del tipo específico
            df_with_speed, df_without_speed = get_fleet_data_by_type(conn, name)
            
            # Agregar el nombre de tipo a la lista de procesados
            processed_type_names.append(name)
        
        # Visualizar los datos combinados para cada tipo procesado
        for name in processed_type_names:
            df_with_speed, df_without_speed = get_fleet_data_by_type(conn, name)
            visualize_fleet_data_combined([df_with_speed], [df_without_speed], name)
        
    finally:
        close_conn(conn)
    
    print("Processed names:", processed_type_names)  # Imprimir los nombres de tipo procesados al final
